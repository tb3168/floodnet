...
# -*- coding: utf-8 -*-
"""
Created on Tue May  6 03:43:30 2025
...
def get_performance_metrics(event_performance):
    tn, fp, fn, tp = confusion_matrix(event_performance["true_label"], event_performance["pred_label"], labels=["noise","flood"]).ravel()
    recall, precision, specificity = performance_metrics(tn, fp, fn, tp)
    return recall, precision, specificity

def evaluate_results(event_performance, label_mappings=None):
    event_performance["label"] = event_performance["event_class"]
    event_performance["cnfsn"] = event_performance.apply(lambda x: get_confusion_label(x["pred_label"],x["true_label"]),axis=1)
